...
namespace CharpZavertailo2.ViewModels.Authentication
{
    class PersonFillInfoViewModel : INotifyPropertyChanged
    {
        ...
        private async void ProceedImpl(object o)
        {
            ...
            if (person.BDate > DateTime.Today)
            {
                MessageBox.Show("You have picked future date");
            }

           else if (person.Years >= 135)
            {
                MessageBox.Show("Oh sorry, but you are too old");
            }
            else
            {
                if (person.IsBirthday)
                {
                    MessageBox.Show($"Happy B-Day to you, Happy B-Day to you, Happy B-Day dear {_name}, Happy B-Day to you!");
                }
